  ...
  useEffect(() => {
    const fetchGastos = async () => {
      try {
        ...
        const gastosUsuario = data.filter((g) => g.idUsuario === usuario.idPersona)
  ...
  useEffect(() => {
    ...
    if (periodType === "dia") {
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        semanaInicio.setDate(now.getDate() - (i + 1) * 7)
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        mes.setMonth(now.getMonth() - i)

        const gastosMes = gastos.filter((g) => {
          const fechaGasto = new Date(g.fecha)
          return fechaGasto.getMonth() === mes.getMonth() && fechaGasto.getFullYear() === mes.getFullYear()
        })
    ...
    setStats({
      ...
      totalIngresos: 0, // Por ahora sin ingresos
      balance: -totalGastos,
    })
  }, [gastos, periodType])
